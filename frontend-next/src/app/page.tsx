...
async function fetchAll(): Promise<LongevityRecord[]> {
  const first = await fetch(`${BASE_URL}/api/v1/analytics?page=0&size=${FETCH_SIZE}&sort=surveyCycle,desc`).then(r => r.json());
  const all: LongevityRecord[] = [...(first.content || [])];
  const totalPages: number = first.totalPages || 1;
  if (totalPages > 1) {
    const rest = await Promise.all(
      Array.from({ length: totalPages - 1 }, (_, i) =>
        fetch(`${BASE_URL}/api/v1/analytics?page=${i + 1}&size=${FETCH_SIZE}&sort=surveyCycle,desc`).then(r => r.json())
      )
    );
      ...
      <div className="border-b border-gray-800 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-end justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-0.5">Nutrición y Riesgo Cardiovascular — Capa Gold</p>
          </div>
          {!loading && (
